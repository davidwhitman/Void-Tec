    ...
    public CommandResult runCommand(@NotNull String args, @NotNull BaseCommand.CommandContext context) {
        ...
        try {
            ...
            for (AugmentCargoWrapper augmentWrapper : CargoUtils.getAugmentsInCargo()) {
                ...
                AugmentApplier augSpec = AugmentDataManager.getAugment(augmentWrapper.getAugment().getAugmentID(), augmentWrapper.getAugment().getAugmentQuality());
                ...
                for (int i = 0; i < primaryValuesFromSpec.size(); i++) {
                    augmentWrapper.getAugment().getPrimaryStatValues()
                            .set(i, AugmentDataLoader.applyStatRangeModifier(primaryValuesFromSpec.get(i), VT_Settings.statRollRangeModifier));
                }
                ...
                for (int i = 0; i < secondaryValuesFromSpec.size(); i++) {
                    augmentWrapper.getAugment().getSecondaryStatValues()
                            .set(i, AugmentDataLoader.applyStatRangeModifier(secondaryValuesFromSpec.get(i), VT_Settings.statRollRangeModifier));
                }
            }
